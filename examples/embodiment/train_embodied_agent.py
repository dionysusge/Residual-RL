...
def main(cfg) -> None:
    ...
    elif cfg.algorithm.loss_type == "residual_ac":
        if use_training_pipeline:
            raise ValueError(
                "runner.use_training_pipeline=True is not supported for residual_ac."
            )
        from rlinf.workers.actor.fsdp_residual_ac_policy_worker import (
            ResidualACFSDPPolicy,
        )

        actor_worker_cls = ResidualACFSDPPolicy
