...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    @torch.no_grad()
    def extract_residual_obs(
        self,
        env_obs: dict[str, Any],
    ) -> dict[str, torch.Tensor]:
        """Return the explicit-residual observation without an RLT token.

        Base actions go through the existing OpenPI inference and output
        transforms. This method intentionally does not require ``use_rlt`` and
        never constructs or fabricates ``z_rl``.
        """
        ref_chunk, _ = self.predict_action_batch(
            env_obs=env_obs,
            mode="eval",
            compute_values=False,
        )
        ...
        if not torch.is_tensor(proprio):
            proprio = torch.as_tensor(proprio)
        return {
            "proprio": proprio.to(device=ref_chunk.device, dtype=torch.float32),
