...
from rlinf.algorithms.residual.transition import update_residual_transitions
...
class EnvWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self.residual_trace_writer = None
        self.residual_trace_buffer: list[dict[str, Any]] = []
        trace_cfg = OmegaConf.select(
            self.cfg, "rollout.residual_trace", default={}
        ) or {}
        self.residual_trace_shard_size = int(trace_cfg.get("shard_size", 32))
        if self.enable_residual and bool(trace_cfg.get("enabled", False)):
            self.residual_trace_writer = AtomicResidualTraceWriter(
                trace_cfg.get("root", "traces"),
                trace_cfg.get(
                    "run_id", self.cfg.runner.logger.experiment_name
                ),
                self._rank,
            )
    ...
    @staticmethod
    def _residual_trace_info(env_output: EnvOutput, key: str) -> Any:
    ...
    def _record_residual_trace(
        ...
        if self.residual_trace_writer is None or policy_output is None:
            return
        inputs = policy_output.forward_inputs
        required = (
            "base_action_chunk",
            "raw_residual",
            "bounded_residual",
        ...
        if any(key not in inputs for key in required):
            return
        reset_id = self._residual_trace_info(env_output, "reset_state_ids")
        record = {
            "run_id": self.cfg.runner.logger.experiment_name,
            "episode_id": reset_id,
            "task_id": self._residual_trace_info(env_output, "task_ids"),
            "reset_state_id": reset_id,
            "seed": int(self.cfg.env.train.seed),
            "policy_version": policy_output.versions,
            "base_action_chunk": inputs["base_action_chunk"],
            "raw_residual": inputs["raw_residual"],
            "bounded_residual": inputs["bounded_residual"],
            "scaled_residual": inputs["scaled_residual"],
            "preclip_action": inputs["preclip_action"],
            "executed_action": inputs["executed_action"],
            "proprio_used": inputs["proprio"],
            "reward": rewards,
            "success": self._residual_trace_info(env_output, "success_at_end"),
            "done": env_output.dones,
            "episode_step": self._residual_trace_info(env_output, "episode_step"),
            "chunk_index": int(chunk_index),
            "residual_std": float(self.cfg.actor.model.residual.std),
            "lambda": float(self.cfg.actor.model.residual.scale),
        }
        self.residual_trace_buffer.append(record)
        if len(self.residual_trace_buffer) >= self.residual_trace_shard_size:
            self.residual_trace_writer.write(self.residual_trace_buffer)
            self.residual_trace_buffer = []
    ...
    async def _run_interact_once(
        ...
        *,
        ...
        residual_pending_obs: list[dict[str, Any] | None] = [None] * self.stage_num
        residual_pending_trace: list[PolicyOutput | None] = [None] * self.stage_num
        ...
        for epoch in range(self.rollout_epoch):
            ...
            for chunk_step_idx in range(self.n_train_chunk_steps):
                for stage_id in range(self.stage_num):
                    ...
                    self._record_residual_trace(
                        residual_pending_trace[stage_id],
                        env_output,
                        rewards,
                        chunk_step_idx,
                    )
                    ...
                    if self.enable_residual and self.collect_transitions:
                        update_residual_transitions(
                            stage_id,
                            residual_pending_obs,
                            self.trajectory_builders,
                            policy_output,
                            cache_current=True,
                        )
                    ...
                    if self.enable_residual:
                        residual_pending_trace[stage_id] = policy_output
                    ...
                    if self.collect_transitions and not (
                        self.enable_rlt or self.enable_residual
                    ):
                        next_obs = (
                            env_output.final_obs
            ...
            for stage_id in range(self.stage_num):
                ...
                if (
                    self.residual_trace_writer is not None
                    and self.residual_trace_buffer
                ):
                    self.residual_trace_writer.write(self.residual_trace_buffer)
                    self.residual_trace_buffer = []
