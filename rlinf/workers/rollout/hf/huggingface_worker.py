...
from rlinf.algorithms.residual import (
    BaseVLAAdapter,
...
from rlinf.envs.residual import LiberoResidualAdapter
...
class MultiStepRolloutWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self.rlt_route = None
        self.residual_base_model = None
        self.residual_benchmark_adapter = None
        self.residual_composer = None
        self.residual_rollout = None
    ...
    def init_worker(self):
        ...
        residual_base_model_config = OmegaConf.select(
            self.cfg, "rollout.residual_base_model", default=None
        )
        if residual_base_model_config is not None:
            frozen_model = get_model(copy.deepcopy(residual_base_model_config))
            frozen_model.eval()
            frozen_model.requires_grad_(False)
            self.residual_base_model = BaseVLAAdapter(frozen_model)
            residual_cfg = self.model_cfg.get("residual", {})
            self.residual_benchmark_adapter = LiberoResidualAdapter(
                action_low=residual_cfg.get("action_low", -1.0),
                action_high=residual_cfg.get("action_high", 1.0),
            )
            self.residual_composer = ResidualActionComposer(
                bound=residual_cfg.get("bound", 0.1),
                dim_mask=residual_cfg.get("dim_mask", 1.0),
            )
            self.residual_rollout = ResidualRollout(
                self.residual_base_model,
                self.residual_benchmark_adapter,
                self.residual_composer,
            )
        ...
        if self.rlt_feature_model is not None:
            self.rlt_feature_model.eval()
        if self.residual_base_model is not None:
            self.residual_base_model.model.eval()
    ...
    def _predict_rollout_actions(
        ...
        if self.residual_base_model is not None:
            return self.residual_rollout.predict(
    ...
    def offload_model(self):
        ...
        if self.residual_base_model is not None:
            self.residual_base_model.model.to("cpu")
        if self.expert_model is not None:
    ...
    def reload_model(self):
        ...
        if self.residual_base_model is not None:
            self.residual_base_model.model.to(self.device)
        if self.expert_model is not None:
